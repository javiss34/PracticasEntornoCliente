...
export const filtrarCorreoYahoo = (usuarios) => {
  return usuarios.filter((usuario) =>
    usuario.contacto.correoelectronico.includes("@yahoo")
  );
};
...
export const filtrarTemaClaroMayorEdadYEspañoles = (usuarios) => {
  ...
  return usuarios.filter(
    (usuario) =>
      usuario.preferencias.tema.toLowerCase() === "claro" &&
...
export const filtrarPorFaltaDatos = (usuarios) => {
  return usuarios.filter((usuario) => {
    ...
    let valores = [
      ...
      ...Object.values(usuario.contacto?.direccion),
    ]; //Con el encadenamiento opcional si contacto no existe devuelve undefined.
